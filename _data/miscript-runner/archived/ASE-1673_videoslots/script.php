...
foreach ($games as $game) {
    $res = $sql->loadArray("SELECT * FROM micro_games WHERE id = {$game['id']} AND game_name = '{$game['game_name']}' AND operator = 'ELK' AND blocked_countries  NOT LIKE '%IS%'");
    if (!empty($res)) {
        $sql->shs()->query("UPDATE micro_games
                    SET blocked_countries = CONCAT(blocked_countries, ' IS')
                    WHERE id = {$game['id']} AND game_name = '{$game['game_name']}' AND operator = 'ELK'");
