...
$csv_path = __DIR__ . "/users_to_remove_blocks_mrv.csv";
$users = readCsv($csv_path);

foreach($users as $user) {
    removeWithdrawalBlock($sc_id, $user['user_id']);
    removeDepositBlock($sc_id, $user['user_id']);
    removePlayBlock($sc_id, $user['user_id']);
}
