...
$csv_path = __DIR__ . "/micro_games_ELK_DBET.csv";
$i = 0;

$games = readCsv($csv_path);

foreach ($games as $game) {
    $res = $sql->loadArray("SELECT * FROM micro_games WHERE id = {$game['id']} AND game_name = '{$game['game_name']}'AND blocked_countries  NOT LIKE '%IS%'");
    if (!empty($res)) {
        $sql->shs()->query("UPDATE micro_games
                    SET blocked_countries = CONCAT(blocked_countries, ' IS')
                    WHERE id = {$game['id']} AND game_name = '{$game['game_name']}'");
