...
class TestEdict extends TestGp
{
    ...
    {
        phive()->dumpTbl("**********************", $p_aParameters);
        $txn = (isset($p_aParameters['transactionid']) ? $p_aParameters['transactionid'] : $this->_getHash());
        $rnd = (isset($p_aParameters['roundid']) ? $p_aParameters['roundid'] : $this->_getHash());
        ...
        return array(
            ...
            'reason' => '',
            'transactionRef' => $txn,
            'gameRoundRef' => $rnd, // PHP_EOL . '<gameRoundRef>{{gameRoundRef}}</gameRoundRef>',
    ...
    private function _parseFile($p_aParams)
    {
        ...
        $sXml = file_get_contents(realpath(dirname(__FILE__)) . '/TestEdict/request/' . substr($this->_m_oGp->setDefaults()->getWalletMethodByGpMethod($this->_m_sGpMethod),
                1) . '.xml');
    ...
    protected function _post($p_sXml)
    {
        ...
        $service = $this->_m_oGp->getSetting('service');
        $xmlnsurl = $this->_m_oGp->getNsUrl();
        $gpmethod = $this->_m_sGpMethod;
