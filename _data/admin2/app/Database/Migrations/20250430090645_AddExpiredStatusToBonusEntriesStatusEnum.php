...
class AddExpiredStatusToBonusEntriesStatusEnum extends Migration
{
        ...
        if ($this->schema->hasTable($this->table)) {
            $this->schema->table($this->table, function (Blueprint $table) {
                ...
                DB::loopNodes(function (\App\Extensions\Database\Connection\Connection $connection) {
                    $connection->statement("ALTER TABLE `{$this->table}` MODIFY COLUMN `status` ENUM('pending', 'active', 'approved', 'completed', 'failed', 'corrupted', 'expired') DEFAULT 'pending' NOT NULL");
                }, true);
            });
        }
    }
