...
class AddPermissionTagsForDeeplTranslatePage extends Seeder
{
    private Connection $connection;
    private string $groupsTable;
    private string $permissionGroupsTable;
    private string $deeplTranslatePermission;

    public function init()
    {
        $this->groupsTable = 'groups';
        $this->permissionGroupsTable = 'permission_groups';
        $this->deeplTranslatePermission = 'admin.translate.deepl';
    ...
    public function up()
    {
        ...
        $permissionGroups = $groupIds->map(function($id) {
            return [
                'group_id' => $id,
                'tag' => $this->deeplTranslatePermission,
                'mod_value' => '',
                'permission' => 'grant',
            ];
        });
    ...
    public function down()
    {
        $this->connection
            ->table($this->permissionGroupsTable)
            ->where('tag', $this->deeplTranslatePermission)
            ->delete();
    }
}
