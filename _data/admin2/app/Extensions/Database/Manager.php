...
class Manager extends BaseManager
{
    ...
    {
        $this->container['config']['database.nodes'] = $nodes;
    }
    ...
    {
        $this->container['config']['database.sharding'] = $config;
    }

    public function setupMigrationDriver(string $driver, $options = [])
    {
        $this->container['config']['database.migration_driver'] = $driver;
        $this->container['config']['database.migration_driver_options'] = $options;
    }
    ...
    /*
     * TODO should add here the cross shards stuff?
    ...
    {
        return static::$instance->getDatabaseManager()->getNodesList();
    }
    ...
    {
        return static::$instance->getDatabaseManager()->getNodes();
    }
    ...
    {
        return static::$instance->getConnection('default');
    }
    ...
    {
        return static::$instance->getConnection(static::$instance->getDatabaseManager()->getShardSelection($shard_key, $table));
    }
     ...
     */
    public static function getAlterConnection(Connection $connection)
    {
        if(static::isAlterConnectionSet()){
            $container_config = static::$instance->getContainer()['config'];

            $connection_config = $connection->getConfig();
            $connection_config['driver'] = $container_config['database.migration_driver'];
            if(!empty($container_config['database.migration_driver_options']) && is_array($container_config['database.migration_driver_options'])){
                $connection_config['params'] = $container_config['database.migration_driver_options'];
            }
            $name = '_alter_' . $connection->getName();
            static::$instance->addConnection($connection_config, $name);
            /** @noinspection CallableParameterUseCaseInTypeContextInspection */
            $connection = static::$instance->getConnection($name);
        }

        return $connection;
    }

    public static function isAlterConnectionSet():bool
    {
        return isset(static::$instance->getContainer()['config']['database.migration_driver']);
    }
    ...
    public static function loopNodes(\Closure $closure, $do_master = false)
    {
        ...
        foreach (self::getNodesList() as $index => $node_name) {
        ...
        if ($do_master === true) {
            $res[] = $closure($connections['default']);
        }
     ...
     * @return mixed
     */
    public static function getEnabledShardSelection($key, $prefix = 'node')
    {
        return static::$instance->getDatabaseManager()->getEnabledShardSelection($key, $prefix);
    }
}
