...
use App\Extensions\Database\ArchiveManager as MyBaseManager;

class ArchiveFManager extends MyBaseManager
{
    /**
    ...
    {
        $this->async = new MysqlAsync(static::$instance->getDatabaseManager()->getArchiveNodes());
    }
    ...
    public static function shTable($key, $table, $fetchMode = null)
    {
        ...
        $archiveConnectionName = $instance->getDatabaseManager()->getArchiveShardSelection($key, preg_split("/[\s]+/", $table)[0]);
        // If $archiveConnectionName returns null then it will connect to default master database. But we need to connect to archive master database.
        $connectionName = $archiveConnectionName == null ?  'videoslots_archived' : $archiveConnectionName;
    ...
    {
        return static::$instance->connection(static::$instance->getDatabaseManager()->getArchiveShardSelection($key, $table))
    ...
    {
        if (archiveDatabaseSwitcher() == false) {
            return FManager::shsSelect($table, $query, $bindings, $params, $closure);
        }
        //todo if it is global do the random thing
        return static::$instance->getConnection('videoslots_archived')->select($query, $bindings, null, static::$instance->getDatabaseManager()->isSharded($table), $params, $closure, false, archiveDatabaseSwitcher());
    }
