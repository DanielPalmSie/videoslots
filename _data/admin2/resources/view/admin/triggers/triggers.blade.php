                    ...
                    <table class="fraud-section-datatable table table-striped table-bordered dt-responsive"
                        ...
                        @foreach($data as $element)
                            <tr>
                                ...
                                <td><input type="text" name="score-{{$element->name}}" value="{{ $element->score}}" id="score-{{$element->name}}"></td>
                                <td><input type="text" name="ngr_threshold-{{$element->name}}" value="<?= $element->ngr_threshold != 0 ? $element->ngr_threshold : 'Not Set'?>" id="ngr_threshold-{{$element->name}}"></td>
        ...
        $(document).ready(function() {
             ...
             $('#triggers-form-submit').on('click', function () {
                $.ajax({
                    ...
                    type: "POST",
                    data: {'params' : $('#trigger-form').serialize()},
