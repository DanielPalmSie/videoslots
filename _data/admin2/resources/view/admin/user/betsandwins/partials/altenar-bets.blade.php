<div class="card card-primary">
    <div class="card-header">
        <h3 class="card-title">
            Altenar Bets and Wins
        </h3>
        <span class="float-right">@include('admin.user.betsandwins.partials.altenar-download-button')</span>
    </div>
    <div class="card-body">
        <div class="table-responsive-sm">
            <table id="user-altenar-bets-datatable"
            ...
            >
                ...
                @foreach($altenar_bets as $p_bet)
                    ...
                    <tr>
                        ...
                        <td>{{ $p_bet->win_id }}</td>
                        <td class="ucbold text-{{ $class }}">{{ $p_bet->ticket_type }}</td>
                        <td>{{ $p_bet->transaction_id }}</td>
                        <td>{{ $p_bet->ext_transaction_id }}</td>
                    <td>{{ $p_bet->bet_reference_num }}</td>
...
<script>
    $(function () {
        ...
        let table = $('#user-altenar-bets-datatable').DataTable({
            paging: true,
            ordering: true,
            columnDefs: [
        ...
        function format(d) {
            let keys = [
                'bet_id', 'bet_date', 'win_id', 'transaction_id', 'ext_transaction_id',
                'transaction_type', 'type', 'amount', 'win_amount'
            ];
            ...
            $.getJSON(`/admin2/altenar-bets/userprofile/${user}/bets-wins/altenar-details/${bet_id}`, function (bet_details) {
        ...
        $('#user-altenar-bets-datatable tbody').on('click', 'td.dt-control', function () {
...
<style>
    #user-altenar-bets-datatable > tbody > tr.shown {
        background-color: #eaeaea;
    }

    #user-altenar-bets-datatable tr.shown ~ tr:not([role]) > td:first-of-type {
        padding: 0;
    }

    #user-altenar-bets-datatable .loading {
        padding: 5px 2px;
    }
